	...
	public function index(Request $request)
	{
		...
		if($request->ajax())
		{
			...
			$data->map(function ($data) {
				...
				$data->ttl_voucher_fh = 'Rp. '.format_uang($data->ttl_voucher);
				$data->total = $data->ttl_transaksi - $data->ttl_voucher;
				$data->total_fh = 'Rp. '.format_uang($data->total);
			...
			->addColumn('aksi', function($data){
				...
				<button type="button" class="detail_transaction btn btn-sm btn-info" data-toggle="modal" data-target="#detail_modal_transaction" name="detail_data_transaction" id="'.$data->id.'" nama="'.$data->nama.'" no_telp="'.$data->no_telp.'" ttl_transaksi="'.$data->ttl_transaksi_fh.'" ttl_voucher="'.$data->ttl_voucher_fh.'" total="'.$data->total_fh.'" tgl_transaksi="'.$data->tgl_transaksi_fh.'" keterangan="'.$data->keterangan.'" ="'.$data->tgl_transaksi_fh.'" title="Detail transaksi">
